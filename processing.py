...
if __name__ == '__main__':
    ...
    Lower = np.array([0, 0, 9])#yellow
    Upper = np.array([27, 163,182])
        ...
        while True:
            ...
            if contours:
                ...
                if startcommend:
                    ...
                    x_array[index]=x
                    #np.append(time_array,time.time())
                    
                    if index>=99:
                        index=-1
                        lowloca = (np.max(x_array)+np.min(x_array))/2
                        print(np.max(x_array),lowloca,np.min(x_array))
                        
                        if np.max(x_array)-lowloca <=20:
                            measurecom=0
                        else:
                            measurecom=1
                        x_array=np.zeros(100)
